...
public interface HBCICallback
{
    ...
    /** Ursache des Callback-Aufrufes: PIN-Eingabe Ã¼ber Computer-Tastatur benÃ¶tigt. Alternativ zum
        ...
        "softwaremÃ¤Ãig" eingegeben werden, d.h. der Anwender gibt die PIN Ã¼ber die PC-Tastatur
        ein, welche Ã¼ber diesen Callback-Aufruf an den HBCI-Kernel Ã¼bergeben wird. Der Kernel
        Ã¼bermittelt die PIN anschlieÃend zur Verifikation an die Chipkarte. In diesem Falle gibt es
    ...
    /** Ursache des Callback-Aufrufes: neue NutzerschlÃ¼ssel generiert (INI-Brief erforderlich). Dieser Callback
        ...
        generiert und zur Bank geschickt hat (und diese die SchlÃ¼sseldaten auf Ãbereinstimmung verglichen
    ...
    /** <p>Ursache des Callback-Aufrufes: Fehler beim Verifizieren einer Kontonummer mit Hilfe
        ...
        <p>Die automatische ÃberprÃ¼fung von Kontonummern findet statt, wenn HBCI-Jobs mit
        ...
        wird die ÃberprÃ¼fung fÃ¼r alle in diesem Job benutzten Kontonummern durchgefÃ¼hrt. FÃ¼r jeden
        PrÃ¼fzifferfehler, der dabei entdeckt wird, wird dieser Callback erzeugt.<br/>
        Tritt beim ÃberprÃ¼fen einer IBAN ein Fehler auf, wird statt dessen
    ...
    /** Ursache des Callback-Aufrufes: Passwort fÃ¼r das Einlesen der SchlÃ¼sseldatei
        ...
        ACHTUNG: Die folgenden Zeichen duerfen NICHT im Passwort enthalten sein: ÃÂ´Â°Â§Ã¼ÃÃ¶Ã¤ÃÃ
        */
    ...
    /** <p>Ursache des Callback-Aufrufes: Auswahl eines Eintrages aus einer SIZ-RDH-Datei
        ...
        werden. Die Callback-Routine muss schlieÃlich die ID des vom Anwender ausgewÃ¤hlten
     ...
     * ausgewÃ¤hlten PIN/TAN-Verfahrens anschlieÃend in <code>retData</code>
     * zurÃ¼ckgeben.</p> */
    ...
    /** Ursache des Callbacks: beim ÃberprÃ¼fen einer IBAN ist ein Fehler aufgetreten.
    ...
    /** Kernel-Status: ÃberprÃ¼fe digitale Signatur der Nachricht. Wird aufgerufen, wenn
     ...
     * gekapselt werden. AuÃerdem muss diese Methode so Ã¼berschrieben werden,
